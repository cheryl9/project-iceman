            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                {issueAreaOptions.map(area => (
                  <button
                    key={area}
                    onClick={() => toggleItem(filters.issueAreas, area, 'issueAreas')}
                    className={`px-3 py-2 rounded-full text-sm transition-all ${
                      filters.issueAreas.includes(area)
              ...
              {filters.issueAreas.length > 0 && (
                <p className="text-xs text-indigo-600 mt-2 font-medium">
                  {filters.issueAreas.length} area{filters.issueAreas.length > 1 ? 's' : ''} selected
                </p>
              )}
            </div>
